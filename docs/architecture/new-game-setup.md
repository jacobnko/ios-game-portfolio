...
### 스토어 스크린샷 — 6.9" 한 벌만 만든다 (Chordline D-108)

App Store Connect는 업로드 시 캔버스 크기를 **정확히** 검사하고 어긋나면 거부한다.
시뮬레이터에서 찍은 원본(예: iPhone 16 Pro는 1206x2480)은 어떤 버킷에도 해당하지 않아
그대로는 못 올린다. 해결책은 더 큰 기기로 다시 찍는 게 아니라, **원본을 캔버스 안의 기기
프레임에 합성**하는 것이다 — 캔버스가 규격이면 원본 해상도는 상관없어진다.

버킷은 **6.9"(1320x2868) 한 벌만** 만든다. iPhone 최대 크기라 Apple이 여기서부터 더 작은
디스플레이로 자동 축소해준다. **축소는 한 방향이다** — App Store Connect가 기본으로 열어주는
슬롯이 6.5"(1284x2778)라 거기부터 채우기 쉬운데, 6.5"를 채우면 6.9"가 빈 채로 남는다.
반대로 6.9"만 채우면 6.5" 이하가 전부 따라온다.

합성 스크립트는 `Apps/Chordline/Marketing/build_frames.py`가 레퍼런스다. 로케일마다
카피와 폰트·자간을 따로 갖고(한글·일본어는 Sora에 글리프가 없어 각자 폰트를 지정한다),
`framed/<locale>/`로 떨어진다. 새 게임은 이 파일을 복사해 `LOCALES`의 카피만 갈아끼운다.

**스크린샷 속 주장은 코드로 확인한 것만 쓴다.** Chordline은 "984 puzzles"와 "모든 판의
정답은 단 하나"를 스테이지 카탈로그와 유일해 검증 테스트로 확인하고 나서 썼다. 화면에 없는
걸 카피가 약속하면 2.3.1 리젝 사유가 되고(결과 화면에 "점화 순간"이라고 쓴 초안이 그랬다),
버튼에 박힌 가격은 전 지역 스토어에 같은 이미지가 쓰이는 탓에 다른 통화권 독자에게 틀린
값을 보여준다 — Chordline은 이 때문에 Remove Ads 버튼에서 가격 표시를 아예 뺐다.
