...
- [x] S3.5 광고 연결 — 힌트 보상형 (클리어 전면은 Phase 2에서 이미 일반화됨, 아래 참고)
  - [x] `HintPlanner` — 힌트 대상 색·경로 선택 (순수, `ChordlineCore`, 테스트 5개)
  - [x] `BoardState.draw(path:)` — 힌트 경로를 `beginPath`/`extend`/`endPath`로 재생 (테스트 5개)
  - [x] `HintCoordinator.requestHint` — 광고 확인 전에 힌트가 있는지 먼저 검사, 성공해야만 보드 반환 (`Views/` 밖, 테스트 6개)
...
- [x] S3.7 **스테이지 100개 이상** (수작업 24 + 생성기 85 = 109개) + 난이도 곡선
  - [x] `HandAuthoredStages` — 검증된 "나란한 직선" 패밀리, 2x2~5x10, `Solver`로 전부 확인 (테스트 3개)
...
- [x] S3.8 **7개 언어 로컬라이즈**, 하드코딩 문자열 0
  - [x] en·ko·ja·de·es-MX·pt-BR·fr — 53개 키 전부. `launchSet`을 전체로 넓혀 기존 완전성 테스트가 강제
  - [x] `CFBundleLocalizations` 7개 선언 — 없으면 iOS가 언어 선택을 안 띄운다(실제 결함이었음). `audit.sh` 검사 추가
...
게임별 항목은 `docs/concepts/game-concepts.md`(비공개)와 **각 게임 저장소의
`checklist.md`**에서 관리한다. 이 저장소는 public이고 `.gitignore`가 게임별 디자인
브리프를 막는 이유(미출시 브랜드·핵심 기전)가 진행 기록에도 그대로 적용된다 —
여기에는 진행 상태와 `CoreKit` 레벨 결정만 남긴다.

- [~] #2 — 착수 2026-09-22. D1~D5 디자인 배달 완료 상태로 시작(화면 게이트 열림).
      상세 스텝은 게임 저장소의 `checklist.md`
  - [ ] `CoreKitJuice`에 실패 연출 타입 신설 — 게임 3~10이 상속 (D-111)
  - [ ] `CoreKitUI` 화면 4종이 두 번째 게임의 디자인을 담는지 실측 + 확장 결정 (D-111)
  - [ ] 컬렉션류 저장이 기존 `StageRecord`에서 파생되는지 판정 (D-111)
- [ ] #3
- [ ] #4
- [ ] #5
- [ ] #6
